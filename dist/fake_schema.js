...
function fakeSchema(schema) {
    var mutationTypeName = (schema.getMutationType() || {}).name;
    ...
    function addFakeProperties(objectType) {
        var isMutation = (objectType.name === mutationTypeName);
    ...
    function getFieldResolver(field) {
        var type = field.type;
        var fakeResolver = getResolver(type, field);
...
function getLeafResolver(type, field) {
